...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	is_correct = 1;
	{
		...
		/*cprintf("expectedva header before realloc. Expected %d\n", *((uint32*)startVAs[blockIndex]-1));
		cprintf("expectedva size before realloc. Expected %d\n", expectedSize);
		 */
		...
		/*
		 * cprintf(" my addr %p , expected addr %p\n", va , expectedVA);
		 * cprintf("expectedva header after realloc. Expected %d\n", *((uint32*)startVAs[blockIndex]-1));
		cprintf("our va header after realloc. Expected %d\n", *((uint32*)va-1));*/
		...
		if (check_block(((uint32*)expectedVA+expectedSize/4), ((uint32*)startVAs[blockIndex]+expectedSize/4), allocSizes[3] + allocSizes[4] - expectedSize, 0) == 0)
		{
			is_correct = 0;
	...
	cprintf("	3.3: reallocate in another place (relocate - split)shimaa\n\n") ;
	...
	*/
	va = alloc_block_FF(4*kilo - sizeOfMetaData);//fill it and then will reempty it , just to meet logic of my code
	*(startVAs[0*allocCntPerSize]) = 0*allocCntPerSize ;
	*(midVAs[0*allocCntPerSize]) = 0*allocCntPerSize ;
	*(endVAs[0*allocCntPerSize]) = 0*allocCntPerSize ;

	free_block(startVAs[6*allocCntPerSize]);
	cprintf("is free %d \n",is_free_block( startVAs[6*allocCntPerSize]));
	cprintf("size 6 before %d \n",get_block_size(startVAs[6*allocCntPerSize]));

	va = realloc_block_FF(startVAs[blockIndex], new_size);
	cprintf("size after %d \n",get_block_size(startVAs[6*allocCntPerSize]));
	cprintf("my va after %p his %p \n",va,startVAs[6*allocCntPerSize]);

	for(int i = 0 ; i < 7 ; i++){
		cprintf("i %d \n",i);
		if(is_free_block( startVAs[i*allocCntPerSize])){
			cprintf("free %d \n",i);
			cprintf("size %d , what you want %d\n",get_block_size(startVAs[i*allocCntPerSize]),expectedSize);

		}
		if(va == startVAs[i*allocCntPerSize]){
			cprintf("hi2 %d \n",i);
		}
	}
